...
/* ═══════════════════════════════════════════════════════════════
   이야기 화로 — THE STORY HEARTH (Final)
   살아있는 불씨 파티클 위에서, 하루의 조각이 글로 구워진다.
   조각이 쌓일수록 화로의 심장이 밝아지고,
   굽는 동안 불씨가 화면을 가득 채운다.
   ═══════════════════════════════════════════════════════════════ */
const COLORS = {
  bg: "#0a0807",
...
const TONES = ["담백하게", "따뜻하게", "위트있게", "차분한 전문가"];
const SPEECHES = ["해요체", "합니다체", "반말(에세이체)"];
const LENGTHS = [
  { id: "short", label: "짧게 · 600~800자", spec: "공백 포함 600~800자" },
  { id: "mid", label: "보통 · 900~1200자", spec: "공백 포함 900~1200자" },
];
const DEFAULT_VOICE = {
  memo: "솔직한 실수와 시행착오를 숨기지 않는 공감형 스토리텔링. 자기비하 유머가 아니라 진심 어린 공개를 통한 공감. 구체적인 한 장면으로 시작해 작은 깨달음으로 끝낸다. 과장된 동기부여 문구 금지.",
  reader: "한인 자영업자, 그리고 노션과 기록을 배우고 싶은 사람들",
  tone: "담백하게",
  speech: "해요체",
  length: "mid",
};
const PLACEHOLDERS = [
  "방금 무슨 일이 있었나요? 한 줄이면 충분해요",
  "지금 스친 생각을 불씨로 남겨두세요",
  "오늘 가장 오래 남을 것 같은 장면 하나",
  "들은 말을, 들은 그대로",
];
...
function voiceSpec(v) {
  const len = LENGTHS.find(l => l.id === v.length) || LENGTHS[1];
...
function EmberCanvas({ stateRef }) {
  ...
  useEffect(() => {
    ...
    function spawn(burst) {
      const heat = stateRef.current.heat; // 0~1 조각 수 기반
      parts.push({
...
export default function App() {
  const [tab, setTab] = useState(0); // 0 화로, 1 서랍, 2 목소리
  ...
  useEffect(() => {
    (async () => {
      ...
      if (f === null) f = await sGet("route-frags", null) || await sGet("press-frags", null) || await sGet("tearoom-frags", null) || [];
      if (e === null) e = await sGet("route-epis", null) || await sGet("press-epis", null) || await sGet("tearoom-epis", null) || [];
      if (v === null) { const ov = await sGet("route-voice", null) || await sGet("press-voice", null); v = ov ? { ...DEFAULT_VOICE, ...ov } : DEFAULT_VOICE; }
      setFrags(f); setEpis(e); setVoice(v); setLoaded(true);
    })();
  }, []);
  ...
  async function bake(withFeedback) {
    ...
    setBrewing(true); setErr("");
    const fragText = included.slice().sort((a, b) => a.ts - b.ts)
      .map(f => `[${dayLabel(f.ts)} ${fmtClock(f.ts)}] ${f.text}`).join("\n");
...
{"titles":["제목후보1","제목후보2","제목후보3"],"draft":"본문 전체"}`;
    try {
      ...
      setDraftEdit(false); setFeedback("");
      setTimeout(() => resultRef.current && resultRef.current.scrollIntoView({ behavior: "smooth", block: "start" }), 150);
    } catch (e) { setErr("불이 잠시 꺼졌어요 — 한 번 더 시도해 주세요"); }
    finally { setBrewing(false); }
  }
...
- 응답은 <svg>로 시작해 </svg>로 끝나는 코드만. 다른 텍스트 절대 금지.`;
    try {
      ...
      setBaked(b => ({ ...b, svg }));
    } catch (e) { setErr("그림이 잘 구워지지 않았어요 — 다시 시도해 주세요"); }
  ...
  function downloadSvg(svg, title) {
    ...
    a.href = url; a.download = (title || "표지") + ".svg"; a.click();
...
반드시 JSON으로만: {"insta":"...","threads":"..."}`;
    ...
    } catch (e) { setErr("변환에 실패했어요 — 다시 시도해 주세요"); }
    finally { setConverting(false); }
  }

  async function copyText(text, msg) {
    try { await navigator.clipboard.writeText(text); toast(msg || "복사했어요"); }
    catch (e) { toast("복사가 막혔어요 — 길게 눌러 직접 복사해 주세요"); }
  }
  function archive() {
    if (!baked) return;
          ...
          <div style={{ marginTop: 22 }}>
            ...
            <div className="card up d2" style={{ marginTop: 13 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {TONES.map(t => <button key={t} className={"chip" + (voice.tone === t ? " on" : "")} onClick={() => setVoice(v => ({ ...v, tone: t }))}>{t}</button>)}
              </div>
              <div className="rk" style={{ marginTop: 20 }}>말투</div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {SPEECHES.map(s => <button key={s} className={"chip" + (voice.speech === s ? " on" : "")} onClick={() => setVoice(v => ({ ...v, speech: s }))}>{s}</button>)}
              </div>
              <div className="rk" style={{ marginTop: 20 }}>분량</div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {LENGTHS.map(l => <button key={l.id} className={"chip" + (voice.length === l.id ? " on" : "")} onClick={() => setVoice(v => ({ ...v, length: l.id }))}>{l.label}</button>)}
              </div>
